...
def append_unique(list, obj):
    ...
    """
    list.append(obj) if obj not in list else None
...
def merge_unique(list_a, list_b):
    ...
    """
    for item in list_b:
        list_a.append(item) if item not in list_a else None
...
def split_integer(list):
    ...
    list_oth=[]
    for item in list:
        list_int.append(item) if isinstance(item, int) else list_oth.append(item)
    return list_int, list_oth
